...
contains
   ...
   subroutine init(this)
      ...
      create_flow_solver: block
         ...
         this%vs=hypre_str(cfg=this%cfg,name='Velocity',method=gmres_pfmg,nst=7)
         ...
         call this%input%read('Ubulk',this%Ubulk)
         call this%input%read('Wbulk',this%Wbulk)
         where (this%fs%umask.eq.0) this%fs%U=this%Ubulk
         where (this%fs%wmask.eq.0) this%fs%W=this%Wbulk
         this%meanU=this%Ubulk
         this%meanW=this%Wbulk
         call this%fs%cfg%sync(this%fs%U)
         call this%fs%cfg%sync(this%fs%V)
         call this%fs%cfg%sync(this%fs%W)
               ...
               do j=this%fs%cfg%jmin_,this%fs%cfg%jmax_
                  do i=this%fs%cfg%imin_,this%fs%cfg%imax_
                     if (this%fs%umask(i,j,k).eq.0) then
                        myU   =myU   +this%fs%cfg%dxm(i)*this%fs%cfg%dy(j)*this%fs%cfg%dz(k)*(2.0_WP*this%fs%U(i,j,k)-this%fs%Uold(i,j,k))
                        myUvol=myUvol+this%fs%cfg%dxm(i)*this%fs%cfg%dy(j)*this%fs%cfg%dz(k)
                     end if
                     if (this%fs%wmask(i,j,k).eq.0) then
                        myW   =myW   +this%fs%cfg%dx(i)*this%fs%cfg%dy(j)*this%fs%cfg%dzm(k)*(2.0_WP*this%fs%W(i,j,k)-this%fs%Wold(i,j,k))
                        myWvol=myWvol+this%fs%cfg%dx(i)*this%fs%cfg%dy(j)*this%fs%cfg%dzm(k)
                     end if
                  end do
               end do
            end do
            call MPI_ALLREDUCE(myUvol,Uvol ,1,MPI_REAL_WP,MPI_SUM,this%fs%cfg%comm,ierr)
            call MPI_ALLREDUCE(myU   ,this%meanU,1,MPI_REAL_WP,MPI_SUM,this%fs%cfg%comm,ierr); this%meanU=this%meanU/Uvol
            where (this%fs%umask.eq.0) this%resU=this%resU+this%Ubulk-this%meanU
            call MPI_ALLREDUCE(myWvol,Wvol ,1,MPI_REAL_WP,MPI_SUM,this%fs%cfg%comm,ierr)
            call MPI_ALLREDUCE(myW   ,this%meanW,1,MPI_REAL_WP,MPI_SUM,this%fs%cfg%comm,ierr); this%meanW=this%meanW/Wvol
            where (this%fs%wmask.eq.0) this%resW=this%resW+this%Wbulk-this%meanW
         end block forcing   
